...
export const addPost: express.RequestHandler = async (
  req: Request,
  res: Response
) => {
  ...
  upload.array("imgs", 3)(req, res, async (error) => {
    ...
    const { post, fk_egroup_id } = req.body;

    if (!req.files || req.files.length === 0) {
      pool.execute<ResultSetHeader>(
        "INSERT INTO egroup_posts (post, fk_egroup_id, fk_user_id) VALUES (?, ?, ?)",
        [post, fk_egroup_id, user_id],
        (error) => {
          if (error) {
            console.error(error);
              ...
              try {
                ...
                connection.query<ResultSetHeader>(
                  ...
                  (error, results) => {
                    ...
                    const fk_egroup_post_id = results.insertId;
                    ...
                    connection.query(
                      "INSERT INTO egroup_post_imgs (fk_egroup_post_id, imgs) VALUES ?",
                      [values],
                      (error) => {
                        if (error) {
